---
Title: Parameter/Layer symbol inventory
Ticket: GL-002-FURTHER-CLEANUP
Status: active
Topics:
    - glazed
    - api-design
    - renaming
    - cleanup
DocType: analysis
Intent: long-term
Owners:
    - manuel
RelatedFiles: []
ExternalSources: []
Summary: ""
LastUpdated: 2026-02-02T20:18:56
WhatFor: ""
WhenToUse: ""
---

# Parameter/Layer Symbol Inventory

Generated: `2026-02-02T20:18:56`

## Scope
This report lists Go identifiers (symbols) containing `parameter` or `layer` (case-insensitive) across non-`ttmp` Go files, plus an index of all non-`ttmp` files where the words appear in contents.

## Summary
- Go files with matching identifiers: **106**
- Unique identifiers found: **319**
- All files with parameter/layer mentions (non-ttmp): **0**
  - Go files: **0**
  - Markdown files: **0**
  - Other files: **0**

## Top identifiers by file coverage
- `layer` — 41 file(s)
- `parsedLayers` — 31 file(s)
- `Parameters` — 26 file(s)
- `glazedLayer` — 20 file(s)
- `Layers` — 20 file(s)
- `ParsedParameters` — 17 file(s)
- `layers_` — 16 file(s)
- `AddLayerToCobraCommand` — 15 file(s)
- `ParsedParameter` — 15 file(s)
- `WithLayersList` — 15 file(s)
- `NewParsedParameters` — 12 file(s)
- `parsedLayer` — 11 file(s)
- `layers` — 10 file(s)
- `defaultLayer` — 9 file(s)
- `TargetLayer` — 9 file(s)
- `TargetParameter` — 8 file(s)
- `cobraLayer` — 7 file(s)
- `ParseParameter` — 7 file(s)
- `CheckParameterDefaultValueValidity` — 6 file(s)
- `GatherParametersFromMap` — 6 file(s)
- `Layer` — 6 file(s)
- `layerSlug` — 5 file(s)
- `NewProfileSettingsLayer` — 5 file(s)
- `parameter` — 5 file(s)
- `ParseLayerFromCobraCommand` — 5 file(s)
- `SkipCommandSettingsLayer` — 5 file(s)
- `WithParameterValue` — 5 file(s)
- `LayerSlug` — 4 file(s)
- `NewGlazedParameterLayers` — 4 file(s)
- `parameterDefinitions` — 4 file(s)
- `parameters_` — 4 file(s)
- `ShortHelpLayers` — 4 file(s)
- `WithValuesForLayers` — 4 file(s)
- `AddParametersToCobraCommand` — 3 file(s)
- `createParameterLayer` — 3 file(s)
- `demoLayer` — 3 file(s)
- `GetAllParsedParameters` — 3 file(s)
- `layerName` — 3 file(s)
- `MergeParameters` — 3 file(s)
- `NewCobraParserFromLayers` — 3 file(s)
- `NewCommandSettingsLayer` — 3 file(s)
- `NewTemplateParameterLayer` — 3 file(s)
- `parameterType` — 3 file(s)
- `paramLayers` — 3 file(s)
- `parsedCommandLayers` — 3 file(s)
- `ParsedParametersFromDefaults` — 3 file(s)
- `psLayer` — 3 file(s)
- `targetParameter` — 3 file(s)
- `testLayers` — 3 file(s)
- `ToSerializableParsedParameters` — 3 file(s)
- `WithParameters` — 3 file(s)
- `WithParsedParameter` — 3 file(s)
- `AddLoggingLayerToRootCommand` — 2 file(s)
- `AppendLayers` — 2 file(s)
- `BlacklistLayerParameters` — 2 file(s)
- `BlacklistLayerParametersFirst` — 2 file(s)
- `BlacklistLayers` — 2 file(s)
- `BlacklistLayersFirst` — 2 file(s)
- `ChildLayers` — 2 file(s)
- `clonedLayers` — 2 file(s)
- `commandSettingsLayer` — 2 file(s)
- `createCommandSettingsLayer` — 2 file(s)
- `createTestLayers` — 2 file(s)
- `EnableCreateCommandSettingsLayer` — 2 file(s)
- `EnableProfileSettingsLayer` — 2 file(s)
- `FieldsFiltersParameterLayer` — 2 file(s)
- `GetDefaultLayer` — 2 file(s)
- `GetDefaultParameterLayer` — 2 file(s)
- `GetLayer` — 2 file(s)
- `GetParameter` — 2 file(s)
- `GlazedTemplateLayerSlug` — 2 file(s)
- `GlazeParameterLayerOption` — 2 file(s)
- `InitializeStructFromParameterDefaults` — 2 file(s)
- `JqParameterLayer` — 2 file(s)
- `layer1` — 2 file(s)
- `layer2` — 2 file(s)
- `LayersToSerializable` — 2 file(s)
- `layerTable` — 2 file(s)
- `LoadParametersFromResolvedFilesForCobra` — 2 file(s)
- `NewBlacklistParameterLayer` — 2 file(s)

## All identifiers (alphabetical)
- `addLayer`
- `AddLayerToCobraCommand`
- `AddLoggingLayerToCommand`
- `AddLoggingLayerToRootCommand`
- `AddParametersToCobraCommand`
- `AppendLayers`
- `argumentParameters`
- `BlacklistedParameters`
- `blacklistedParameters`
- `BlacklistLayerParameters`
- `BlacklistLayerParametersFirst`
- `BlacklistLayerParametersHandler`
- `BlacklistLayers`
- `BlacklistLayersFirst`
- `BlacklistLayersHandler`
- `BlacklistParameterLayer`
- `bootstrapLayers`
- `buildTestLayers`
- `cfgLayer`
- `CheckParameterDefaultValueValidity`
- `ChildLayers`
- `clonedLayers`
- `cloneLayers`
- `cobraLayer`
- `commandSettingsLayer`
- `commandSettingsLayers`
- `createCommandSettingsLayer`
- `createLayer`
- `createParameterLayer`
- `createSimpleParameterLayer`
- `createTestLayers`
- `dbLayer`
- `defaultLayer`
- `defaultLayer_`
- `defaultParameterLayer`
- `demoLayer`
- `EnableCreateCommandSettingsLayer`
- `enableCreateCommandSettingsLayer`
- `EnableProfileSettingsLayer`
- `enableProfileSettingsLayer`
- `ExpectedArgumentParameters`
- `ExpectedFlagParameters`
- `expectedLayers`
- `ExpectedLayers`
- `expectedLayers_`
- `FieldsFiltersParameterLayer`
- `fieldsFiltersParameterLayer`
- `flagParameters`
- `GatherParametersFromMap`
- `gatherParametersYAML`
- `GetAllParsedParameters`
- `getCliopatraParameters`
- `GetDefaultLayer`
- `GetDefaultParameterLayer`
- `GetLayer`
- `GetParameter`
- `glazedLayer`
- `glazedLayers`
- `glazedMinimalCommandLayer`
- `GlazedParameterLayers`
- `GlazedTemplateLayerSlug`
- `GlazeParameterLayerOption`
- `glLayer`
- `InitializeDefaultsFromParameters`
- `InitializeStructFromParameterDefaults`
- `initialParameterTests`
- `JqParameterLayer`
- `jqParameterLayer`
- `layer`
- `Layer`
- `layer0`
- `layer1`
- `layer1Duplicate`
- `layer2`
- `layer3`
- `layerData`
- `layerKey`
- `layerMap`
- `layerMulti`
- `layerName`
- `layerPrefix`
- `Layers`
- `layers`
- `layers1`
- `layers2`
- `layers_`
- `LayerSlug`
- `layerSlug`
- `layersMap`
- `layersTable`
- `layersToDelete`
- `layersToMerge`
- `LayersToSerializable`
- `layersToUpdate`
- `layerTable`
- `layerToMerge`
- `LoadParametersFromFile`
- `LoadParametersFromResolvedFilesForCobra`
- `loadParameterTypeTests`
- `loggingLayer`
- `LoggingLayerSlug`
- `makeParsedDefaultLayer`
- `MergeParameters`
- `minimalLayer`
- `NewBlacklistParameterLayer`
- `NewCobraParserFromLayers`
- `NewCommandSettingsLayer`
- `NewCreateCommandSettingsLayer`
- `NewFieldsFiltersParameterLayer`
- `NewGlazedParameterLayers`
- `NewJqParameterLayer`
- `NewJqSettingsFromParameters`
- `newLayer`
- `newLayers`
- `NewLoggingLayer`
- `NewOutputParameterLayer`
- `NewParameterTypesCommand`
- `NewParsedParameters`
- `NewProfileSettingsLayer`
- `NewRenameParameterLayer`
- `NewRenameSettingsFromParameters`
- `NewReplaceParameterLayer`
- `NewReplaceSettingsFromParameters`
- `NewSelectParameterLayer`
- `NewSelectSettingsFromParameters`
- `NewSkipLimitParameterLayer`
- `NewSkipLimitSettingsFromParameters`
- `NewSortParameterLayer`
- `NewSortSettingsFromParameters`
- `NewTemplateParameterLayer`
- `NewTestParameterLayer`
- `NewTestParameterLayers`
- `newTestRedisLayer`
- `NewWhitelistParameterLayer`
- `numLayers`
- `originalLayers`
- `OutputParameterLayer`
- `outputParameterLayer`
- `Parameter`
- `parameter`
- `parameterData`
- `parameterDefinition`
- `parameterDefinitions`
- `ParameterDefinitionToDict`
- `ParameterDefs`
- `parameterLayers`
- `ParameterLayers`
- `parameterName`
- `Parameters`
- `parameters`
- `parameters_`
- `parametersToKeep`
- `ParameterTest`
- `ParameterTestCase`
- `parameterType`
- `ParameterTypesCommand`
- `ParameterTypesSettings`
- `parameterTypeToJsonSchema`
- `paramLayers`
- `ParseCommandParameters`
- `ParseCommandSettingsLayer`
- `parsedCommandLayers`
- `parsedLayer`
- `parsedLayer1`
- `parsedLayer2`
- `parsedLayers`
- `parsedParameter`
- `ParsedParameter`
- `ParsedParameters`
- `parsedParameters`
- `ParsedParametersFromDefaults`
- `ParsedParametersOption`
- `ParseLayerFromCobraCommand`
- `ParseLuaTableToLayer`
- `ParseParameter`
- `ParseParameterFromLua`
- `PrependLayers`
- `PrintParsedParameters`
- `printParsedParameters`
- `printParsedParameters_`
- `profileSettingsLayer`
- `psLayer`
- `readConfigFileToLayerMap`
- `redisLayer`
- `RenameParameterLayer`
- `renameParameterLayer`
- `ReplaceParameterLayer`
- `replaceParameterLayer`
- `resolveCanonicalParameterName`
- `ResolveTargetParameter`
- `resolveTargetParameter`
- `sameDefaultLayer`
- `sameLayer`
- `SelectParameterLayer`
- `selectParameterLayer`
- `SerializableLayers`
- `SerializableParameterLayer`
- `SerializableParsedParameter`
- `SerializableParsedParameters`
- `SetLayers`
- `setupLayers`
- `shortHelperLayer`
- `ShortHelpLayers`
- `shortHelpLayers`
- `shortHelpLayers_`
- `SkipCommandSettingsLayer`
- `skipCommandSettingsLayer`
- `SkipLimitParameterLayer`
- `skipLimitParameterLayer`
- `SortParameterLayer`
- `sortParameterLayer`
- `TargetLayer`
- `targetLayer`
- `TargetParameter`
- `targetParameter`
- `TemplateParameterLayer`
- `templateParameterLayer`
- `TestBlacklistLayerParameters`
- `TestBlacklistLayerParametersFirst`
- `TestBlacklistLayers`
- `TestBlacklistLayersFirst`
- `TestExpectedLayer`
- `TestGatherArguments_ListParameterParsing`
- `TestGatherFlagsFromStringList_ValidArgumentsAndParameters`
- `TestGatherParametersFromMap`
- `TestInitializeStructWithMissingParameters`
- `TestIntegrationWithLoadParametersFromFile`
- `TestLayerPrefix`
- `testLayers`
- `testLayersMulti`
- `TestNewParameterLayers`
- `TestParameterDate`
- `testParameterDefinitions`
- `TestParameterLayer`
- `TestParameterLayersAppendLayers`
- `TestParameterLayersAsList`
- `TestParameterLayersClone`
- `TestParameterLayersForEach`
- `TestParameterLayersForEachE`
- `TestParameterLayersGetAllDefinitions`
- `TestParameterLayersMerge`
- `TestParameterLayersMergeWithOverlappingLayers`
- `TestParameterLayersPrependLayers`
- `TestParameterLayersSubset`
- `TestParameterLayersSubsetWithNonExistentLayers`
- `TestParameterLayersWithDuplicateSlugs`
- `TestParameterLayersWithLargeNumberOfLayers`
- `TestParameterLayersWithLayers`
- `TestParameterLayersWithUnicodeLayerNames`
- `testParametersTypesYaml`
- `TestParameterTypes`
- `testParameterTypeTests`
- `TestParameterValidity`
- `testParameterValidList`
- `TestParsedParameter`
- `TestParsedParametersFromDefaults_BasicTypes`
- `TestParsedParametersFromDefaults_EdgeCases`
- `TestParsedParametersFromDefaults_EmptyCollections`
- `TestParsedParametersFromDefaults_FileLoadingTypes`
- `TestParsedParametersFromDefaults_ListTypes`
- `TestParsedParametersFromDefaults_MapTypes`
- `TestParsedParametersFromDefaults_NilComplexTypes`
- `TestParseParameter`
- `TestResolveTargetParameter`
- `TestSingleLayer`
- `TestSingleParametersFollowedByListDefaults`
- `TestThreeSingleParametersFollowedByListDefaults`
- `TestThreeSingleParametersFollowedByListDefaultsOnlyTwoValues`
- `TestValuesGetAllParsedParameters`
- `TestValuesGetDefaultParameterLayer`
- `TestValuesGetOrCreateNilLayer`
- `TestValuesGetParameter`
- `TestValuesGetParameterNonExistentLayer`
- `TestWhitelistLayerParameters`
- `TestWhitelistLayerParametersFirst`
- `TestWhitelistLayers`
- `TestWhitelistLayersFirst`
- `TestWrapWithRestrictedLayers`
- `ToSerializableParsedParameter`
- `ToSerializableParsedParameters`
- `toUrlParameter`
- `ValuesForLayers`
- `WhitelistedParameters`
- `whitelistedParameters`
- `WhitelistLayerParameters`
- `WhitelistLayerParametersFirst`
- `WhitelistLayerParametersHandler`
- `WhitelistLayers`
- `WhitelistLayersFirst`
- `WhitelistLayersHandler`
- `WhitelistParameterLayer`
- `WithCobraShortHelpLayers`
- `WithCreateCommandSettingsLayer`
- `WithFieldsFiltersParameterLayerOptions`
- `WithJqParameterLayerOptions`
- `WithLayers`
- `WithLayersList`
- `WithLayersMap`
- `WithOutputParameterLayerOptions`
- `WithParameters`
- `WithParameterValue`
- `WithParsedParameter`
- `WithProfileSettingsLayer`
- `WithRenameParameterLayerOptions`
- `WithReplaceLayers`
- `WithReplaceParameterLayerOptions`
- `WithSelectParameterLayerOptions`
- `WithSkipCommandSettingsLayer`
- `WithSkipLimitParameterLayerOptions`
- `WithSortParameterLayerOptions`
- `WithTemplateParameterLayerOptions`
- `WithValuesForLayers`
- `WrapWithBlacklistedLayers`
- `WrapWithBlacklistedParameterLayers`
- `WrapWithLayerModifyingHandler`
- `wrapWithRestrictedLayersTest`
- `wrapWithRestrictedLayersTestsYAML`
- `WrapWithWhitelistedLayers`
- `WrapWithWhitelistedParameterLayers`

## Per-file identifier inventory (Go)
### `cmd/examples/appconfig-parser/main.go`
- `LayerSlug`
- `WithValuesForLayers`
- `dbLayer`
- `redisLayer`

### `cmd/examples/appconfig-profiles/main.go`
- `AddLayerToCobraCommand`
- `NewProfileSettingsLayer`
- `addLayer`
- `cobraLayer`
- `layer`
- `psLayer`
- `redisLayer`

### `cmd/examples/config-custom-mapper/main.go`
- `Layers`
- `SkipCommandSettingsLayer`
- `WithLayersList`
- `demoLayer`
- `layer`
- `layerSlug`
- `parsedCommandLayers`

### `cmd/examples/config-overlay/main.go`
- `Layers`
- `SkipCommandSettingsLayer`
- `WithLayersList`
- `layer`
- `layerSlug`

### `cmd/examples/config-pattern-mapper/main.go`
- `TargetLayer`
- `TargetParameter`
- `demoLayer`
- `paramLayers`

### `cmd/examples/config-single/main.go`
- `Layers`
- `SkipCommandSettingsLayer`
- `WithLayersList`
- `layer`
- `layerSlug`

### `cmd/examples/middlewares-config-env/main.go`
- `WithLayersList`

### `cmd/examples/new-api-build-first-command/main.go`
- `ShortHelpLayers`

### `cmd/examples/overlay-override/main.go`
- `WithLayersList`

### `cmd/examples/parameter-types/main.go`
- `NewParameterTypesCommand`
- `ParameterTypesCommand`
- `ParameterTypesSettings`
- `WithLayersList`
- `parameterData`

### `cmd/examples/sources-example/main.go`
- `Layers`

### `cmd/glaze/cmds/csv.go`
- `WithLayersList`
- `glazedLayer`

### `cmd/glaze/cmds/docs.go`
- `AddLayerToCobraCommand`
- `WithFieldsFiltersParameterLayerOptions`
- `cobraLayer`
- `glazedLayer`

### `cmd/glaze/cmds/example.go`
- `WithLayersList`
- `glazedLayer`

### `cmd/glaze/cmds/html/cmds.go`
- `AddLayerToCobraCommand`
- `cobraLayer`
- `glazedLayer`

### `cmd/glaze/cmds/json.go`
- `WithLayersList`
- `glazedLayer`

### `cmd/glaze/cmds/markdown.go`
- `AddLayerToCobraCommand`
- `cobraLayer`
- `glazedLayer`

### `cmd/glaze/cmds/yaml.go`
- `WithLayersList`
- `glazedLayer`

### `cmd/glaze/main.go`
- `AddLoggingLayerToRootCommand`

### `pkg/appconfig/options.go`
- `NewProfileSettingsLayer`
- `WithValuesForLayers`
- `bootstrapLayers`
- `layers_`
- `parsedLayers`
- `psLayer`

### `pkg/appconfig/parser.go`
- `LayerSlug`
- `layer`
- `paramLayers`
- `parsedLayers`

### `pkg/appconfig/parser_test.go`
- `AddLayerToCobraCommand`
- `LayerSlug`
- `WithValuesForLayers`
- `cobraLayer`
- `layer`
- `newTestRedisLayer`

### `pkg/appconfig/profile_test.go`
- `AddLayerToCobraCommand`
- `LayerSlug`
- `NewProfileSettingsLayer`
- `layer`
- `newTestRedisLayer`
- `psLayer`

### `pkg/cli/cli.go`
- `LoadParametersFromFile`
- `NewCommandSettingsLayer`
- `NewCreateCommandSettingsLayer`
- `NewProfileSettingsLayer`
- `PrintParsedParameters`
- `createCommandSettingsLayer`
- `glazedMinimalCommandLayer`
- `profileSettingsLayer`

### `pkg/cli/cliopatra/capture.go`
- `Layers`
- `Parameter`
- `Parameters`
- `ParsedParameters`
- `getCliopatraParameters`
- `layer`
- `parameters_`
- `parsedLayer`
- `parsedLayers`

### `pkg/cli/cliopatra/capture_test.go`
- `GetLayer`
- `Layer`
- `NewParsedParameters`
- `Parameters`
- `ParsedParameters`
- `TestSingleLayer`
- `WithLayersList`
- `WithParsedParameter`
- `defaultLayer`
- `layer`
- `makeParsedDefaultLayer`

### `pkg/cli/cliopatra/program.go`
- `GetAllParsedParameters`
- `Parameter`
- `ParsedParameters`
- `parsedLayers`
- `parsedParameter`

### `pkg/cli/cobra-parser.go`
- `AddLayerToCobraCommand`
- `EnableCreateCommandSettingsLayer`
- `EnableProfileSettingsLayer`
- `Layers`
- `LoadParametersFromResolvedFilesForCobra`
- `NewCobraParserFromLayers`
- `NewCommandSettingsLayer`
- `NewCreateCommandSettingsLayer`
- `NewProfileSettingsLayer`
- `ParseCommandSettingsLayer`
- `ShortHelpLayers`
- `SkipCommandSettingsLayer`
- `cobraLayer`
- `commandSettingsLayer`
- `commandSettingsLayers`
- `createCommandSettingsLayer`
- `enableCreateCommandSettingsLayer`
- `enableProfileSettingsLayer`
- `layer`
- `paramLayers`
- `parsedCommandLayers`
- `parsedLayers`
- `profileSettingsLayer`
- `shortHelpLayers`
- `shortHelperLayer`
- `skipCommandSettingsLayer`

### `pkg/cli/cobra.go`
- `EnableCreateCommandSettingsLayer`
- `EnableProfileSettingsLayer`
- `Layers`
- `NewCobraParserFromLayers`
- `NewGlazedParameterLayers`
- `PrintParsedParameters`
- `ShortHelpLayers`
- `SkipCommandSettingsLayer`
- `WithCobraShortHelpLayers`
- `WithCreateCommandSettingsLayer`
- `WithProfileSettingsLayer`
- `WithSkipCommandSettingsLayer`
- `createLayer`
- `glLayer`
- `glazedLayer`
- `glazedLayers`
- `layers`
- `layers_`
- `minimalLayer`
- `originalLayers`
- `parsedLayers`
- `printParsedParameters`
- `printParsedParameters_`

### `pkg/cli/helpers.go`
- `AddLayerToCobraCommand`
- `GlazeParameterLayerOption`
- `NewCobraParserFromLayers`
- `NewGlazedParameterLayers`
- `Parameters`
- `ParsedParameter`
- `layer`
- `layerName`
- `layersMap`
- `layers_`
- `parameter`
- `parsedLayer`
- `parsedLayers`
- `printParsedParameters`

### `pkg/cmds/alias/alias.go`
- `Layers`
- `newLayers`
- `parsedLayers`

### `pkg/cmds/cmds.go`
- `GetDefaultLayer`
- `GetLayer`
- `Layers`
- `ParsedParametersFromDefaults`
- `SetLayers`
- `WithLayers`
- `WithLayersList`
- `WithLayersMap`
- `WithReplaceLayers`
- `cloneLayers`
- `layer`
- `layers`
- `layers_`
- `parsedLayers`

### `pkg/cmds/cobra_test.go`
- `AddLayerToCobraCommand`
- `AddParametersToCobraCommand`
- `ExpectedArgumentParameters`
- `ExpectedFlagParameters`
- `GetDefaultLayer`
- `Layers`
- `ParsedParameters`
- `WithLayersList`
- `argumentParameters`
- `defaultLayer`
- `defaultLayer_`
- `flagParameters`
- `layer`

### `pkg/cmds/fields/cobra.go`
- `AddParametersToCobraCommand`
- `CheckParameterDefaultValueValidity`
- `NewParsedParameters`
- `ParseParameter`
- `ParsedParameter`
- `ParsedParameters`
- `parameter`

### `pkg/cmds/fields/gather-arguments.go`
- `NewParsedParameters`
- `ParseParameter`
- `ParsedParameter`
- `ParsedParameters`

### `pkg/cmds/fields/gather-arguments_test.go`
- `TestGatherArguments_ListParameterParsing`
- `TestSingleParametersFollowedByListDefaults`
- `TestThreeSingleParametersFollowedByListDefaults`
- `TestThreeSingleParametersFollowedByListDefaultsOnlyTwoValues`

### `pkg/cmds/fields/gather-parameters.go`
- `GatherParametersFromMap`
- `NewParsedParameters`
- `ParseParameter`
- `ParsedParameter`
- `ParsedParameters`

### `pkg/cmds/fields/gather-parameters_test.go`
- `GatherParametersFromMap`
- `ParameterDefs`
- `TestGatherParametersFromMap`
- `gatherParametersYAML`
- `parsedParameters`

### `pkg/cmds/fields/initialize-struct.go`
- `ParsedParameter`
- `ParsedParameters`
- `parameter`

### `pkg/cmds/fields/initialize-struct_test.go`
- `NewParsedParameters`
- `ParsedParameters`
- `TestInitializeStructWithMissingParameters`
- `WithParsedParameter`

### `pkg/cmds/fields/parameters.go`
- `CheckParameterDefaultValueValidity`
- `NewParsedParameters`
- `ParsedParameters`
- `ParsedParametersFromDefaults`
- `parameter`
- `parameterDefinitions`
- `parameterType`
- `parameters`
- `parameters_`
- `parsedParameters`

### `pkg/cmds/fields/parameters_from_defaults_test.go`
- `ParsedParametersFromDefaults`
- `TestParsedParametersFromDefaults_BasicTypes`
- `TestParsedParametersFromDefaults_EdgeCases`
- `TestParsedParametersFromDefaults_EmptyCollections`
- `TestParsedParametersFromDefaults_FileLoadingTypes`
- `TestParsedParametersFromDefaults_ListTypes`
- `TestParsedParametersFromDefaults_MapTypes`
- `TestParsedParametersFromDefaults_NilComplexTypes`
- `parameterDefinitions`

### `pkg/cmds/fields/parameters_test.go`
- `TestParameterTypes`
- `TestParameterValidity`
- `initialParameterTests`
- `loadParameterTypeTests`
- `parameterDefinition`
- `testParameterDefinitions`
- `testParameterTypeTests`
- `testParameterValidList`
- `testParametersTypesYaml`

### `pkg/cmds/fields/parse.go`
- `ParseParameter`
- `ParsedParameter`

### `pkg/cmds/fields/parse_test.go`
- `CheckParameterDefaultValueValidity`
- `ParameterTest`
- `ParameterTestCase`
- `ParseParameter`
- `TestParameterDate`
- `TestParseParameter`
- `parameter`

### `pkg/cmds/fields/parsed-parameter.go`
- `NewParsedParameters`
- `ParsedParameter`
- `ParsedParameters`
- `ParsedParametersOption`
- `ToSerializableParsedParameters`
- `WithParsedParameter`
- `parameterType`

### `pkg/cmds/fields/serialize.go`
- `Parameters`
- `ParsedParameter`
- `ParsedParameters`
- `SerializableParsedParameter`
- `SerializableParsedParameters`
- `ToSerializableParsedParameter`
- `ToSerializableParsedParameters`

### `pkg/cmds/fields/strings.go`
- `NewParsedParameters`
- `ParseParameter`
- `ParsedParameter`
- `ParsedParameters`

### `pkg/cmds/fields/strings_test.go`
- `ParsedParameter`
- `TestGatherFlagsFromStringList_ValidArgumentsAndParameters`

### `pkg/cmds/fields/viper.go`
- `NewParsedParameters`
- `ParsedParameter`
- `ParsedParameters`

### `pkg/cmds/helpers/test-helpers.go`
- `BlacklistLayerParameters`
- `BlacklistLayerParametersFirst`
- `BlacklistLayers`
- `BlacklistLayersFirst`
- `Layers`
- `NewParsedParameters`
- `NewTestParameterLayer`
- `NewTestParameterLayers`
- `Parameters`
- `TestBlacklistLayerParameters`
- `TestBlacklistLayerParametersFirst`
- `TestBlacklistLayers`
- `TestBlacklistLayersFirst`
- `TestExpectedLayer`
- `TestParameterLayer`
- `TestParsedParameter`
- `TestWhitelistLayerParameters`
- `TestWhitelistLayerParametersFirst`
- `TestWhitelistLayers`
- `TestWhitelistLayersFirst`
- `WhitelistLayerParameters`
- `WhitelistLayerParametersFirst`
- `WhitelistLayers`
- `WhitelistLayersFirst`
- `WithParameters`
- `expectedLayers`
- `expectedLayers_`
- `parsedLayers`

### `pkg/cmds/json-schema.go`
- `parameterTypeToJsonSchema`

### `pkg/cmds/logging/layer.go`
- `AddLoggingLayerToCommand`
- `AddLoggingLayerToRootCommand`
- `Layers`
- `LoggingLayerSlug`
- `NewLoggingLayer`
- `loggingLayer`
- `parsedLayers`

### `pkg/cmds/runner/run.go`
- `Layers`
- `ParseCommandParameters`
- `ValuesForLayers`
- `WithValuesForLayers`
- `glazedLayer`
- `parsedLayers`

### `pkg/cmds/schema/cobra.go`
- `AddLayerToCobraCommand`
- `ParseLayerFromCobraCommand`

### `pkg/cmds/schema/layer-impl.go`
- `AddLayerToCobraCommand`
- `AddParametersToCobraCommand`
- `CheckParameterDefaultValueValidity`
- `ChildLayers`
- `GatherParametersFromMap`
- `InitializeDefaultsFromParameters`
- `InitializeStructFromParameterDefaults`
- `Layer`
- `Parameters`
- `ParseLayerFromCobraCommand`
- `ParsedParameters`
- `parameterDefinitions`

### `pkg/cmds/schema/layer-impl_test.go`
- `AddLayerToCobraCommand`
- `createSimpleParameterLayer`
- `layer`

### `pkg/cmds/schema/layer.go`
- `AddLayerToCobraCommand`
- `AppendLayers`
- `CheckParameterDefaultValueValidity`
- `LayersToSerializable`
- `Parameters`
- `PrependLayers`
- `layers`
- `parsedLayer`
- `parsedLayers`

### `pkg/cmds/schema/layer_test.go`
- `AppendLayers`
- `PrependLayers`
- `TestNewParameterLayers`
- `TestParameterLayersAppendLayers`
- `TestParameterLayersAsList`
- `TestParameterLayersClone`
- `TestParameterLayersForEach`
- `TestParameterLayersForEachE`
- `TestParameterLayersGetAllDefinitions`
- `TestParameterLayersMerge`
- `TestParameterLayersMergeWithOverlappingLayers`
- `TestParameterLayersPrependLayers`
- `TestParameterLayersSubset`
- `TestParameterLayersSubsetWithNonExistentLayers`
- `TestParameterLayersWithDuplicateSlugs`
- `TestParameterLayersWithLargeNumberOfLayers`
- `TestParameterLayersWithLayers`
- `TestParameterLayersWithUnicodeLayerNames`
- `createParameterLayer`
- `layer`
- `layer0`
- `layer1`
- `layer1Duplicate`
- `layer2`
- `layer3`
- `layers`
- `layers1`
- `layers2`
- `numLayers`

### `pkg/cmds/schema/serialize.go`
- `Layers`
- `LayersToSerializable`
- `Parameters`
- `SerializableLayers`
- `SerializableParameterLayer`
- `layer`
- `layers`

### `pkg/cmds/schema/wrappers.go`
- `BlacklistParameterLayer`
- `BlacklistedParameters`
- `NewBlacklistParameterLayer`
- `NewWhitelistParameterLayer`
- `WhitelistParameterLayer`
- `WhitelistedParameters`
- `blacklistedParameters`
- `layer`
- `whitelistedParameters`

### `pkg/cmds/sources/cobra.go`
- `LoadParametersFromResolvedFilesForCobra`
- `Parameters`
- `ParseLayerFromCobraCommand`
- `cobraLayer`
- `defaultLayer`
- `layers_`
- `parameterDefinitions`
- `parsedCommandLayers`
- `parsedLayer`
- `parsedLayers`

### `pkg/cmds/sources/custom-profiles_test.go`
- `Parameters`
- `layer`
- `parameterLayers`
- `parsedLayer`
- `parsedLayers`

### `pkg/cmds/sources/layers.go`
- `MergeParameters`
- `layer`
- `layerSlug`
- `layerToMerge`
- `layersToMerge`
- `layers_`
- `newLayer`
- `newLayers`
- `parsedLayers`
- `targetLayer`

### `pkg/cmds/sources/load-parameters-from-json.go`
- `layerData`
- `layerMap`
- `layerSlug`
- `layers_`
- `parsedLayers`
- `readConfigFileToLayerMap`

### `pkg/cmds/sources/middlewares.go`
- `clonedLayers`
- `layers`
- `layers_`
- `parsedLayers`

### `pkg/cmds/sources/middlewares_test.go`
- `ExpectedLayers`
- `NewTestParameterLayers`
- `ParameterLayers`
- `TestExpectedLayer`
- `TestParameterLayer`
- `TestWrapWithRestrictedLayers`
- `WrapWithBlacklistedLayers`
- `WrapWithWhitelistedLayers`
- `layers`
- `layers_`
- `parsedLayers`
- `wrapWithRestrictedLayersTest`
- `wrapWithRestrictedLayersTestsYAML`

### `pkg/cmds/sources/patternmapper/exports.go`
- `ResolveTargetParameter`
- `resolveTargetParameter`
- `targetParameter`

### `pkg/cmds/sources/patternmapper/loader.go`
- `TargetLayer`
- `TargetParameter`
- `layers_`

### `pkg/cmds/sources/patternmapper/pattern_mapper.go`
- `TargetLayer`
- `TargetParameter`
- `layer`
- `layers`
- `resolveCanonicalParameterName`
- `resolveTargetParameter`
- `targetParameter`

### `pkg/cmds/sources/patternmapper/pattern_mapper_builder.go`
- `TargetLayer`
- `TargetParameter`
- `layers`
- `targetLayer`
- `targetParameter`

### `pkg/cmds/sources/patternmapper/pattern_mapper_edge_cases_test.go`
- `TargetLayer`
- `TargetParameter`
- `TestLayerPrefix`
- `createTestLayers`
- `layer`
- `setupLayers`
- `testLayers`

### `pkg/cmds/sources/patternmapper/pattern_mapper_loader_test.go`
- `TargetLayer`
- `buildTestLayers`

### `pkg/cmds/sources/patternmapper/pattern_mapper_orderedmap_test.go`
- `TargetLayer`
- `TargetParameter`
- `layer`

### `pkg/cmds/sources/patternmapper/pattern_mapper_proposals_test.go`
- `TargetLayer`
- `TargetParameter`
- `layer`
- `layerMulti`
- `testLayers`
- `testLayersMulti`

### `pkg/cmds/sources/patternmapper/pattern_mapper_test.go`
- `ResolveTargetParameter`
- `TargetLayer`
- `TargetParameter`
- `TestIntegrationWithLoadParametersFromFile`
- `TestResolveTargetParameter`
- `createTestLayers`
- `demoLayer`
- `layers_`
- `testLayers`

### `pkg/cmds/sources/profiles.go`
- `layers_`
- `parsedLayers`

### `pkg/cmds/sources/update.go`
- `GatherParametersFromMap`
- `Parameters`
- `ParseParameter`
- `layer`
- `layerPrefix`
- `layers_`
- `parsedLayer`
- `parsedLayers`

### `pkg/cmds/sources/update_test.go`
- `Parameters`
- `cfgLayer`
- `layer`

### `pkg/cmds/sources/whitelist.go`
- `BlacklistLayerParameters`
- `BlacklistLayerParametersFirst`
- `BlacklistLayerParametersHandler`
- `BlacklistLayers`
- `BlacklistLayersFirst`
- `BlacklistLayersHandler`
- `NewBlacklistParameterLayer`
- `NewWhitelistParameterLayer`
- `WhitelistLayerParameters`
- `WhitelistLayerParametersFirst`
- `WhitelistLayerParametersHandler`
- `WhitelistLayers`
- `WhitelistLayersFirst`
- `WhitelistLayersHandler`
- `WrapWithBlacklistedLayers`
- `WrapWithBlacklistedParameterLayers`
- `WrapWithLayerModifyingHandler`
- `WrapWithWhitelistedLayers`
- `WrapWithWhitelistedParameterLayers`
- `clonedLayers`
- `layersToDelete`
- `layersToUpdate`
- `layers_`
- `parametersToKeep`
- `parameters_`
- `parsedLayers`

### `pkg/cmds/template.go`
- `Layers`
- `WithLayersList`
- `defaultLayer`
- `parsedLayers`

### `pkg/cmds/values/parsed-layer.go`
- `GetAllParsedParameters`
- `GetDefaultParameterLayer`
- `GetParameter`
- `Layer`
- `MergeParameters`
- `NewParsedParameters`
- `Parameters`
- `ParsedParameter`
- `ParsedParameters`
- `WithParameterValue`
- `WithParameters`
- `defaultLayer`
- `defaultParameterLayer`
- `layer`
- `layerKey`
- `parameters_`

### `pkg/cmds/values/parsed-layer_test.go`
- `GetAllParsedParameters`
- `GetDefaultParameterLayer`
- `GetParameter`
- `Layer`
- `TestValuesGetAllParsedParameters`
- `TestValuesGetDefaultParameterLayer`
- `TestValuesGetOrCreateNilLayer`
- `TestValuesGetParameter`
- `TestValuesGetParameterNonExistentLayer`
- `WithParameterValue`
- `createParameterLayer`
- `defaultLayer`
- `layer`
- `layer1`
- `layer2`
- `parsedLayer`
- `parsedLayer1`
- `parsedLayer2`
- `parsedLayers`
- `sameDefaultLayer`
- `sameLayer`

### `pkg/cmds/values/serialize_parsed.go`
- `Layer`
- `Layers`
- `Parameters`
- `SerializableParsedParameters`
- `ToSerializableParsedParameters`

### `pkg/cmds/values/test_helpers_test.go`
- `Parameters`
- `ParsedParameter`
- `createParameterLayer`
- `layer`

### `pkg/codegen/glazed.go`
- `CheckParameterDefaultValueValidity`
- `ParameterDefinitionToDict`
- `parameterType`

### `pkg/help/cmd/cobra.go`
- `shortHelpLayers`
- `shortHelpLayers_`

### `pkg/helpers/maps/maps.go`
- `parameterName`

### `pkg/helpers/templating/templating.go`
- `toUrlParameter`

### `pkg/lua/cmds.go`
- `Layers`
- `glazedLayer`
- `layer`
- `layerName`
- `layerTable`
- `layersTable`
- `parsedLayers`

### `pkg/lua/lua.go`
- `GatherParametersFromMap`
- `MergeParameters`
- `Parameters`
- `ParseLuaTableToLayer`
- `ParseParameterFromLua`
- `ParsedParameter`
- `WithParameters`
- `layer`
- `layerName`
- `layerTable`
- `layers_`
- `parameterLayers`
- `parsedLayer`
- `parsedLayers`

### `pkg/settings/glazed_layer.go`
- `AddLayerToCobraCommand`
- `ChildLayers`
- `FieldsFiltersParameterLayer`
- `GatherParametersFromMap`
- `GlazeParameterLayerOption`
- `GlazedParameterLayers`
- `JqParameterLayer`
- `Layer`
- `NewFieldsFiltersParameterLayer`
- `NewGlazedParameterLayers`
- `NewJqParameterLayer`
- `NewJqSettingsFromParameters`
- `NewOutputParameterLayer`
- `NewParsedParameters`
- `NewRenameParameterLayer`
- `NewRenameSettingsFromParameters`
- `NewReplaceParameterLayer`
- `NewReplaceSettingsFromParameters`
- `NewSelectParameterLayer`
- `NewSelectSettingsFromParameters`
- `NewSkipLimitParameterLayer`
- `NewSkipLimitSettingsFromParameters`
- `NewSortParameterLayer`
- `NewSortSettingsFromParameters`
- `NewTemplateParameterLayer`
- `OutputParameterLayer`
- `Parameters`
- `ParseLayerFromCobraCommand`
- `ParsedParameters`
- `RenameParameterLayer`
- `ReplaceParameterLayer`
- `SelectParameterLayer`
- `SkipLimitParameterLayer`
- `SortParameterLayer`
- `TemplateParameterLayer`
- `WithFieldsFiltersParameterLayerOptions`
- `WithJqParameterLayerOptions`
- `WithOutputParameterLayerOptions`
- `WithRenameParameterLayerOptions`
- `WithReplaceParameterLayerOptions`
- `WithSelectParameterLayerOptions`
- `WithSkipLimitParameterLayerOptions`
- `WithSortParameterLayerOptions`
- `WithTemplateParameterLayerOptions`
- `fieldsFiltersParameterLayer`
- `glazedLayer`
- `jqParameterLayer`
- `layer`
- `layers`
- `outputParameterLayer`
- `renameParameterLayer`
- `replaceParameterLayer`
- `selectParameterLayer`
- `skipLimitParameterLayer`
- `sortParameterLayer`
- `templateParameterLayer`

### `pkg/settings/settings_fields-filters.go`
- `AddLayerToCobraCommand`
- `FieldsFiltersParameterLayer`
- `InitializeStructFromParameterDefaults`
- `NewFieldsFiltersParameterLayer`
- `Parameters`
- `ParseLayerFromCobraCommand`
- `ParsedParameter`
- `glazedLayer`
- `layer`

### `pkg/settings/settings_jq.go`
- `JqParameterLayer`
- `NewJqParameterLayer`
- `NewJqSettingsFromParameters`
- `Parameters`
- `glazedLayer`
- `layer`

### `pkg/settings/settings_output.go`
- `NewOutputParameterLayer`
- `OutputParameterLayer`
- `Parameters`
- `glazedLayer`
- `layer`

### `pkg/settings/settings_rename.go`
- `NewRenameParameterLayer`
- `NewRenameSettingsFromParameters`
- `Parameters`
- `RenameParameterLayer`
- `glazedLayer`
- `layer`

### `pkg/settings/settings_replace.go`
- `NewReplaceParameterLayer`
- `NewReplaceSettingsFromParameters`
- `Parameters`
- `ReplaceParameterLayer`
- `glazedLayer`
- `layer`

### `pkg/settings/settings_select.go`
- `NewSelectParameterLayer`
- `NewSelectSettingsFromParameters`
- `Parameters`
- `SelectParameterLayer`
- `glazedLayer`
- `layer`

### `pkg/settings/settings_skip_limit.go`
- `NewSkipLimitParameterLayer`
- `NewSkipLimitSettingsFromParameters`
- `Parameters`
- `SkipLimitParameterLayer`
- `glazedLayer`
- `layer`

### `pkg/settings/settings_sort.go`
- `NewSortParameterLayer`
- `NewSortSettingsFromParameters`
- `Parameters`
- `SortParameterLayer`
- `glazedLayer`
- `layer`

### `pkg/settings/settings_template.go`
- `GlazedTemplateLayerSlug`
- `NewTemplateParameterLayer`
- `Parameters`
- `TemplateParameterLayer`
- `layer`

### `pkg/settings/settings_template_test.go`
- `GlazedTemplateLayerSlug`
- `NewTemplateParameterLayer`
- `layer`
- `layers_`
- `parsedLayers`

### `ttmp/2025-07-29/test_defaults/test_defaults.go`
- `Layers`
- `WithParameterValue`
- `defaultLayer`
- `parsedLayer`
- `parsedLayers`

### `ttmp/2025-07-29/test_doc_example/test_doc_example.go`
- `Layers`
- `WithParameterValue`
- `defaultLayer`
- `parsedLayer`
- `parsedLayers`

### `ttmp/2025-07-29/test_template_api/test_template_api.go`
- `Layers`
- `WithParameterValue`
- `defaultLayer`
- `parsedLayer`
- `parsedLayers`

### `ttmp/2025-08-05/signal-test/main.go`
- `NewCommandSettingsLayer`
- `NewGlazedParameterLayers`
- `ShortHelpLayers`
- `WithLayersList`
- `commandSettingsLayer`
- `glazedLayer`
- `parsedLayers`

## File index with parameter/layer mentions (non-ttmp)
### Go files

### Markdown files

### Other files

