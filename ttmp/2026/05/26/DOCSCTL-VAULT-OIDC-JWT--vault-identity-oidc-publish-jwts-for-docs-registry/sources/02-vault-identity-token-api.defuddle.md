---
Title: 02 Vault Identity Token Api.Defuddle
DocType: source
...
- [`issuer`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#issuer) `(string: "")` – Issuer URL to be used in the iss claim of the token. If not set, Vault's api\_addr will be used. The issuer is a case sensitive URL using the https scheme that contains scheme, host, and an optional port number.
...
- [`name`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#name) `(string)` – Name of the named key.
- [`rotation_period`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#rotation_period) `(int or time string: "24h")` - How often to generate a new signing key. Uses [duration format strings](https://developer.hashicorp.com/vault/docs/concepts/duration-format).
- [`verification_ttl`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#verification_ttl) `(int or time string: "24h")` - Controls how long the public portion of a signing key will be available for verification after being rotated. Uses [duration format strings](https://developer.hashicorp.com/vault/docs/concepts/duration-format).
- [`allowed_client_ids`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#allowed_client_ids) `(list: [])` - Array of role client ids allowed to use this key for signing. If empty, no roles are allowed. If "\*", all roles are allowed.
- [`algorithm`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#algorithm) `(string: "RS256")` - Signing algorithm to use. Allowed values are: RS256 (default), RS384, RS512, ES256, ES384, ES512, EdDSA.
...
- [`name`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#name-1) `(string)` – Name of the key.
...
- [`name`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#name-2) `(string)` – Name of the key.
...
- [`name`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#name-3) `(string)` – Name of the key to be rotated.
- [`verification_ttl`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#verification_ttl-1) `(string: <optional>)` - Controls how long the public portion of the key will be available for verification after being rotated. Setting verification\_ttl here will override the verification\_ttl set on the key.
...
- [`name`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#name-4) `(string)` – Name of the role.
- [`key`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#key) `(string)` – A configured named key, the key must already exist.
- [`template`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#template) `(string: <optional>)` - The template string to use for generating tokens. This may be in string-ified JSON or base64 format.
- [`client_id`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#client_id) `(string: <optional>)` - Optional client ID. A random ID will be generated if left unset.
- [`ttl`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#ttl) `(int or time string: "24h")` - TTL of the tokens generated against the role. Uses [duration format strings](https://developer.hashicorp.com/vault/docs/concepts/duration-format).
...
- [`name`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#name-5) `(string)` – Name of the role.
...
- [`name`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#name-6) `(string)` – Name of the role.
...
- [`name`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#name-7) `(string: "")` – The name of the role against which to generate a signed ID token
...
- [`token`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#token) `(string)` – A signed OIDC compliant ID token
- [`client_id`](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#client_id-1) `(string: <optional>)` - Specifying the client ID additionally requires the token to contain a matching `aud` claim
...
Use the `.well-known` endpoint to retrieve an [OpenID Provider Configuration Response](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderConfigurationResponse) with a set of claims about the identity token issuer.
...
Use the `.well-known` endpoint to retrieve an [OpenID Provider Configuration Response](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderConfigurationResponse) with a set of claims about the plugin identity token issuer.
...
## Read OpenID configuration for secret sync identity token issuer Enterprise

Use the `.well-known` endpoint to retrieve an [OpenID Provider Configuration Response](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderConfigurationResponse) with a set of claims about the secret sync identity token issuer.
...
## Read the public JWKS from a secrets-sync identity token issuer Enterprise
...
[Edit this page on GitHub](https://github.com/hashicorp/web-unified-docs/blob/main/content/vault/v2.x/content/api-docs/secret/identity/tokens.mdx)