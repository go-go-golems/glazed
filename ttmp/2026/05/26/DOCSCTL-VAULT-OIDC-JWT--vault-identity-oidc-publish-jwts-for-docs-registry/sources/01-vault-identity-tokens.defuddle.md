---
Title: 01 Vault Identity Tokens.Defuddle
DocType: source
...
Identity information is used throughout Vault, but it can also be exported for use by other applications. An authorized user/application can request a token that encapsulates identity information for their associated entity. These tokens are signed JWTs following the [OIDC ID token](https://openid.net/specs/openid-connect-core-1_0.html#IDToken) structure. The public keys used to authenticate the tokens are published by Vault on an unauthenticated endpoint following OIDC discovery and JWKS conventions, which should be directly usable by JWT/OIDC libraries. An introspection endpoint is also provided by Vault for token verification.
...
- [`iss`](https://developer.hashicorp.com/vault/docs/secrets/identity/identity-token#iss) - Issuer URL
- [`sub`](https://developer.hashicorp.com/vault/docs/secrets/identity/identity-token#sub) - Requester's entity ID
- [`aud`](https://developer.hashicorp.com/vault/docs/secrets/identity/identity-token#aud) - `client_id` for the role
- [`iat`](https://developer.hashicorp.com/vault/docs/secrets/identity/identity-token#iat) - Time of issue
- [`exp`](https://developer.hashicorp.com/vault/docs/secrets/identity/identity-token#exp) - Expiration time for the token

In addition, the operator may configure per-role templates that allow a variety of other entity information to be added to the token. The templates are structured as JSON with replaceable parameters. The parameter syntax is the same as that used for [ACL Path Templating](https://developer.hashicorp.com/vault/docs/concepts/policies).
...
An authenticated client may request a token using the [token generation endpoint](https://developer.hashicorp.com/vault/api-docs/secret/identity/tokens#generate-a-signed-id-token). The token will be generated per the requested role's specifications, for the requester's entity. It is not possible to generate tokens for an arbitrary entity.
