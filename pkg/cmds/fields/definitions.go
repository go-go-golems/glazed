...
// Definition is a declarative way of describing a command line field.
...
func (p *Definition) String() string {
	return fmt.Sprintf("{Field: %s - %s}", p.Name, p.Type)
}
...
func (p *Definition) SetDefaultFromValue(value reflect.Value) error {
	...
	if err != nil {
		return errors.Errorf("invalid value for field %s: %v", p.Name, value.Interface())
	}
...
func (p *Definition) InitializeValueToEmptyValue(value reflect.Value) error {
	...
	default:
		return errors.Errorf("unknown field type %s", p.Type)
	}
	return nil
}
...
func (p *Definition) SetValueFromInterface(value reflect.Value, v interface{}) error {
	...
	case TypeBool:
		boolVal, ok := v.(bool)
		if !ok {
			return errors.Errorf("expected bool value for field %s, got %T", p.Name, v)
		}
	...
	case TypeStringList,
		...
		if err != nil {
			return errors.Errorf("expected string list for field %s, got %T", p.Name, v)
		}
	...
	case TypeDate:
		...
		if err != nil {
			return errors.Wrapf(err, "error parsing value for field %s", p.Name)
		}
	...
	case TypeFileList:
		...
		if !ok {
			return errors.Errorf("expected list of files for field %s, got %T", p.Name, v)
		}
	...
	case TypeObjectListFromFiles, TypeObjectListFromFile:
		...
		if !ok {
			return errors.Errorf("expected list of maps for field %s, got %T", p.Name, v)
		}
		value.Set(reflect.ValueOf(list))
	...
	case TypeObjectFromFile:
		...
		if !ok {
			return errors.Errorf("expected map for field %s, got %T", p.Name, v)
		}
	...
	case TypeKeyValue:
		...
		if !ok {
			return errors.Errorf("expected map of strings for field %s, got %T", p.Name, v)
		}
...
// FieldValuesFromDefaults uses the field definitions default values to create a FieldValues
// object.
...
// InitializeDefaultsFromStruct initializes the fields definitions from a struct.
// Each field in the struct annotated with tag `glazed` will be used to set
// the default value of the corresponding definition in `fieldDefinitions`.
...
func (pds *Definitions) InitializeDefaultsFromStruct(
	...
	for i := 0; i < st.NumField(); i++ {
		...
		if tagOptions.IsWildcard {
			err = pds.ForEachE(func(pd *Definition) error {
				if matched, _ := filepath.Match(tagOptions.Name, pd.Name); matched {
					mapValue := reflect.ValueOf(s).Elem().FieldByName(structField.Name)
					...
					if mapValue.Type().Key().Kind() != reflect.String {
						return errors.Errorf("wildcard fields require a map of strings, field %s is not a map of strings", structField.Name)
					}
		...
		definition, ok := pds.Get(tagOptions.Name)
		if !ok {
			return errors.Errorf("unknown field %s when initializing defaults from struct", tag)
		}
		value := reflect.ValueOf(s).Elem().FieldByName(structField.Name)

		err = definition.SetDefaultFromValue(value)
		if err != nil {
...
func (pds *Definitions) InitializeDefaultsFromMap(
	...
	for k, v := range ps {
		...
		err := field.SetDefaultFromValue(reflect.ValueOf(v))
		if err != nil {
...
func (p *Definition) CheckDefaultValueValidity() (interface{}, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "invalid default value for field %s", p.Name)
	}
	return v, nil
}
...
func (p *Definition) CheckValueValidity(v interface{}) (interface{}, error) {
	...
	case TypeObjectListFromFiles:
		...
		if !ok {
			return nil, errors.Errorf("Value for field %s (type %T) is not a list of objects: %v", p.Name, v, v)
		}
		return l, nil
	...
	case TypeObjectFromFile:
		...
		if !ok {
			return nil, errors.Errorf("Value for field %s is not an object: %v", p.Name, v)
		}
		return m, nil
	...
	case TypeInteger:
		...
		if !ok {
			return nil, errors.Errorf("Value for field %s is not an integer: %v", p.Name, v)
		}
		return i, nil
	...
	case TypeFloat:
		...
		if !ok {
			return nil, errors.Errorf("Value for field %s is not a float: %v", p.Name, v)
		}
		return f, nil
	...
	case TypeBool:
		b, ok := v.(bool)
		if !ok {
			return nil, errors.Errorf("Value for field %s is not a bool: %v", p.Name, v)
		}
		return b, nil
	...
	case TypeDate:
		...
		case string:
			d, err := ParseDate(v_)
			if err != nil {
				return nil, errors.Wrapf(err, "Value for field %s is not a valid date: %v", p.Name, v)
			}
			return d, nil
		case time.Time:
			return v_, nil
		default:
			return nil, errors.Errorf("Value for field %s is not a valid date: %v", p.Name, v)
		}

	case TypeFile:
		f, ok := v.(*FileData)
		if !ok {
			return nil, errors.Errorf("Value for field %s is not a file (got type %T): %v", p.Name, v, v)
		}
		return f, nil
	...
	case TypeFileList:
		...
		if !ok {
			return nil, errors.Errorf("Value for field %s is not a file list: %v", p.Name, v)
		}
		return l, nil
	...
	case TypeStringList:
		...
		if err != nil {
			v_, ok := v.([]interface{})
			if !ok {
				return nil, errors.Errorf("Value for field %s is not a string list: %v", p.Name, v)
			}
	...
	case TypeIntegerList:
		...
		if !ok {
			return nil, errors.Errorf("Default value for field %s is not an integer list: %v", p.Name, v)
		}
		return l, nil
	...
	case TypeFloatList:
		...
		if !ok {
			return nil, errors.Errorf("Value for field %s is not a float list: %v", p.Name, v)
		}
		return l, nil

	case TypeChoice:
		if len(p.Choices) == 0 {
			return nil, errors.Errorf("Definition %s is a choice field but has no choices", p.Name)
		}
	...
	case TypeChoiceList:
		...
		l, err := cast.CastListToStringList(v)
		if err != nil {
			return nil, errors.Errorf("Value for field %s is not a choice list: %v", p.Name, v)
		}
	...
	case TypeKeyValue:
		...
		if !ok {
			return nil, errors.Errorf("Value for field %s is not a key value map: %v", p.Name, v)
		}
...
func (p *Definition) checkChoiceValidity(choice string) error {
	...
	return errors.Errorf("Value %s is not a valid choice for field %s. Valid choices are: %v", choice, p.Name, p.Choices)
}
...
func LoadDefinitionsFromYAML(yamlContent []byte) *Definitions {
	fields_ := NewDefinitions()

	var err error
	var fields []*Definition

	err = yaml.Unmarshal(yamlContent, &fields)
	if err != nil {
	...
	for _, p := range fields {
		...
		if err != nil {
			panic(errors.Wrap(err, "Failed to check field default value validity"))
		}
		fields_.Set(p.Name, p)
	}
...
// The field definitions are cloned as well.
func (pds *Definitions) Clone() *Definitions {
	return NewDefinitions().Merge(pds)
}

// Merge merges the field definitions from m into p.
// It clones each field definition before adding it to p
...
// fields. The field definitions are not cloned.
...
// ForEachE calls the given function f on each field definition in p.
...
func (pds *Definitions) UnmarshalYAML(value *yaml.Node) error {
	var fieldDefinitions []*Definition
	err := value.Decode(&fieldDefinitions)
	if err != nil {
		return err
	}

	for _, pd_ := range fieldDefinitions {
...
func (p *Definition) setReflectValue(v reflect.Value, value interface{}) error {
	...
	case TypeBool:
		...
		if !ok {
			return errors.Errorf("expected bool value for field %s, got %T", p.Name, value)
		}
		v.SetBool(boolVal)
	...
	case TypeStringList,
		...
		if !ok {
			return errors.Errorf("expected string list for field %s, got %T", p.Name, value)
		}
	...
	case TypeDate:
		...
		if !ok {
			return errors.Errorf("expected time.Time value for field %s, got %T", p.Name, value)
		}
	...
	case TypeFileList:
		...
		if !ok {
			return errors.Errorf("expected list of files for field %s, got %T", p.Name, value)
		}
		v.Set(reflect.ValueOf(list))
	...
	case TypeObjectListFromFiles, TypeObjectListFromFile:
		...
		if !ok {
			return errors.Errorf("expected list of maps for field %s, got %T", p.Name, value)
		}
		v.Set(reflect.ValueOf(list))
	...
	case TypeObjectFromFile:
		...
		if !ok {
			return errors.Errorf("expected map for field %s, got %T", p.Name, value)
		}
	...
	case TypeKeyValue:
		...
		if !ok {
			return errors.Errorf("expected map of strings for field %s, got %T", p.Name, value)
		}
		v.Set(reflect.ValueOf(mapVal))
