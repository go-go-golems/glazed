...
//go:embed "test-data/definitions_test.yaml"
var testFlagsYaml []byte

var testFieldDefinitions *Definitions
...
//go:embed "test-data/definitions_validity_test.yaml"
var validityTestYaml []byte

var testFieldValidList []*ValidityTest
...
func loadValidityTestDataFromYAML(s []byte) ([]*ValidityTest, error) {
	...
	for _, test := range tests {
		test.fieldDefinition = &Definition{
			Name:     test.Name,
			Type:     test.Type,
			Default:  nil,
...
func loadTypeTests(yamlData []byte) ([]TypeTest, error) {
	var tests []TypeTest
...
var testTypeTests []TypeTest

func initialFieldTests() {
	testFieldDefinitions = LoadDefinitionsFromYAML(testFlagsYaml)
	var err error
	testFieldValidList, err = loadValidityTestDataFromYAML(validityTestYaml)
	if err != nil {
		panic(err)
	}

	testTypeTests, err = loadTypeTests(testFieldsTypesYaml)
	if err != nil {
		panic(err)
	}
}
...
func TestTypes(t *testing.T) {
	initialFieldTests()
	for _, test := range testTypeTests {
		t.Run(test.Type, func(t *testing.T) {
			type_ := Type(test.Type)
...
func TestFieldValidity(t *testing.T) {
	initialFieldTests()
	for _, validityTest := range testFieldValidList {
		t.Run(validityTest.Name, func(t *testing.T) {
			_, err := validityTest.fieldDefinition.CheckValueValidity(validityTest.Value)
...
func TestSetValueFromDefaultInt(t *testing.T) {
	initialFieldTests()
	intFlag, _ := testFieldDefinitions.Get("int-flag")
	...
	intFlag, _ = testFieldDefinitions.Get("int-flag-without-default")
	err = intFlag.SetValueFromDefault(iValue)
	require.NoError(t, err)
	assert.Equal(t, 0, i)
	...
	intFlag, _ = testFieldDefinitions.Get("int-flag-with-empty-default")
	err = intFlag.SetValueFromDefault(iValue)
	require.NoError(t, err)
	assert.Equal(t, i, 0)
}
...
func TestSetValueFromDefaultFloat(t *testing.T) {
	initialFieldTests()
	floatFlag, _ := testFieldDefinitions.Get("float-flag")
	...
	floatFlag, _ = testFieldDefinitions.Get("float-flag-without-default")
	err = floatFlag.SetValueFromDefault(fValue)
	require.NoError(t, err)
	...
	floatFlag, _ = testFieldDefinitions.Get("float-flag-with-empty-default")
	err = floatFlag.SetValueFromDefault(fValue)
	require.NoError(t, err)
	assert.Equal(t, 0.0, f)

	floatFlag, _ = testFieldDefinitions.Get("float-flag-with-int-default")
	err = floatFlag.SetValueFromDefault(fValue)
	require.NoError(t, err)
...
func TestSetValueFromDefaultDate(t *testing.T) {
	initialFieldTests()
	dateFlag, _ := testFieldDefinitions.Get("date-flag")
	...
	dateFlag, _ = testFieldDefinitions.Get("date-flag-without-default")
	err = dateFlag.SetValueFromDefault(dValue)
	require.NoError(t, err)
...
func TestSetValueFromDefaultString(t *testing.T) {
	initialFieldTests()
	stringFlag, _ := testFieldDefinitions.Get("string-flag")
	...
	stringFlag, _ = testFieldDefinitions.Get("string-flag-without-default")
	err = stringFlag.SetValueFromDefault(sValue)
	require.NoError(t, err)
	assert.Equal(t, "", s)
	...
	stringFlag, _ = testFieldDefinitions.Get("string-flag-with-empty-default")
	err = stringFlag.SetValueFromDefault(sValue)
	require.NoError(t, err)
	assert.Equal(t, "", s)
}
...
func TestSetValueFromDefaultBool(t *testing.T) {
	boolFlag, _ := testFieldDefinitions.Get("bool-flag")
	...
	boolFlag, _ = testFieldDefinitions.Get("bool-flag-without-default")
	err = boolFlag.SetValueFromDefault(bValue)
	require.NoError(t, err)
	...
	boolFlag, _ = testFieldDefinitions.Get("bool-flag-with-empty-default")
	err = boolFlag.SetValueFromDefault(bValue)
	require.NoError(t, err)
	assert.Equal(t, false, b)
}
...
func TestSetValueFromDefaultChoice(t *testing.T) {
	choiceFlag, _ := testFieldDefinitions.Get("choice-flag")
	...
	choiceFlag, _ = testFieldDefinitions.Get("choice-flag-without-default")
	err = choiceFlag.SetValueFromDefault(cValue)
	require.NoError(t, err)
	assert.Equal(t, "", c)

	choiceFlag = &Definition{
...
func TestSetValueFromDefaultChoiceList(t *testing.T) {
	choiceListFlag, _ := testFieldDefinitions.Get("choice-list-flag")
	...
	choiceListFlag, _ = testFieldDefinitions.Get("choice-list-flag-without-default")
	err = choiceListFlag.SetValueFromDefault(clValue)
	require.NoError(t, err)
	assert.Equal(t, []string{}, cl)

	choiceListFlag = &Definition{
...
func TestSetValueFromDefaultIntList(t *testing.T) {
	intListFlag, _ := testFieldDefinitions.Get("int-list-flag")
	...
	intListFlag, _ = testFieldDefinitions.Get("int-list-flag-without-default")
	err = intListFlag.SetValueFromDefault(ilValue)
	require.NoError(t, err)
	...
	intListFlag, _ = testFieldDefinitions.Get("int-list-flag-with-empty-default")
	err = intListFlag.SetValueFromDefault(ilValue)
	require.NoError(t, err)
	assert.Equal(t, []int{}, il)
}
...
func TestSetValueFromDefaultFloatList(t *testing.T) {
	floatListFlag, _ := testFieldDefinitions.Get("float-list-flag")
	...
	floatListFlag, _ = testFieldDefinitions.Get("float-list-flag-without-default")
	err = floatListFlag.SetValueFromDefault(flValue)
	require.NoError(t, err)
	...
	floatListFlag, _ = testFieldDefinitions.Get("float-list-flag-with-empty-default")
	err = floatListFlag.SetValueFromDefault(flValue)
	require.NoError(t, err)
	assert.Equal(t, []float64{}, fl)
}
...
func TestSetValueFromDefaultObjectFromFile(t *testing.T) {
	objectFromFileFlag, _ := testFieldDefinitions.Get("object-from-file-flag")
	...
	objectFromFileFlag, _ = testFieldDefinitions.Get("object-from-file-flag-without-default")
	err = objectFromFileFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	...
	objectFromFileFlag, _ = testFieldDefinitions.Get("object-from-file-flag-with-empty-default")
	err = objectFromFileFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	assert.Equal(t, map[string]interface{}{}, fl)
}
...
func TestSetValueFromDefaultObjectListFromFile(t *testing.T) {
	objectListFromFileFlag, _ := testFieldDefinitions.Get("object-list-from-file-flag")
	...
	objectListFromFileFlag, _ = testFieldDefinitions.Get("object-list-from-file-flag-without-default")
	err = objectListFromFileFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	assert.Equal(t, []map[string]interface{}{}, fl)
	...
	objectListFromFileFlag, _ = testFieldDefinitions.Get("object-list-from-file-flag-with-empty-default")
	err = objectListFromFileFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	assert.Equal(t, []map[string]interface{}{}, fl)
}
...
func TestSetValueFromDefaultStringFromFile(t *testing.T) {
	stringFromFileFlag, _ := testFieldDefinitions.Get("string-from-file-flag")
	...
	stringFromFileFlag, _ = testFieldDefinitions.Get("string-from-file-flag-without-default")
	err = stringFromFileFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	assert.Equal(t, "", fl)
	...
	stringFromFileFlag, _ = testFieldDefinitions.Get("string-from-file-flag-with-empty-default")
	err = stringFromFileFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	assert.Equal(t, "", fl)
}
...
func TestSetValueFromDefaultStringListFromFile(t *testing.T) {
	stringListFromFileFlag, _ := testFieldDefinitions.Get("string-list-from-file-flag")
	...
	stringListFromFileFlag, _ = testFieldDefinitions.Get("string-list-from-file-flag-without-default")
	err = stringListFromFileFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	...
	stringListFromFileFlag, _ = testFieldDefinitions.Get("string-list-from-file-flag-with-empty-default")
	err = stringListFromFileFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	assert.Equal(t, []string{}, fl)
}
...
func TestSetValueFromDefaultStringListAlias(t *testing.T) {
	initialFieldTests()
	stringListFlag, _ := testFieldDefinitions.Get("string-list-flag")
	...
	stringListFlag, _ = testFieldDefinitions.Get("string-list-flag-without-default")
	err = stringListFlag.SetValueFromDefault(slValue)
	require.NoError(t, err)
	...
	stringListFlag, _ = testFieldDefinitions.Get("string-list-flag-with-empty-default")
	err = stringListFlag.SetValueFromDefault(slValue)
	require.NoError(t, err)
	assert.Equal(t, []StringAlias{}, sl)
}
...
func TestSetValueFromDefaultKeyValue(t *testing.T) {
	keyValueFlag, _ := testFieldDefinitions.Get("key-value-flag")
	...
	keyValueFlag, _ = testFieldDefinitions.Get("key-value-flag-without-default")
	err = keyValueFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	...
	keyValueFlag, _ = testFieldDefinitions.Get("key-value-flag-with-empty-default")
	err = keyValueFlag.SetValueFromDefault(oValue)
	require.NoError(t, err)
	assert.Equal(t, map[string]string{}, fl)
}
