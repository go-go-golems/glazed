...
var testParameterDefinitions *Definitions
...
func loadValidityTestDataFromYAML(s []byte) ([]*ValidityTest, error) {
	...
	for _, test := range tests {
		test.parameterDefinition = &Definition{
			Name:     test.Name,
			Type:     test.Type,
			Default:  nil,
...
func loadParameterTypeTests(yamlData []byte) ([]TypeTest, error) {
	var tests []TypeTest
...
var testParameterTypeTests []TypeTest

func initialParameterTests() {
	testParameterDefinitions = LoadDefinitionsFromYAML(testFlagsYaml)
	var err error
