...
type TestCase struct {
	...
	ParameterDefs []*ParameterDefinition `yaml:"parameterDefs"`
...
//go:embed test-data/gather-parameters.yaml
...
func TestGatherParametersFromMap(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.Title, func(t *testing.T) {
			pds := NewParameterDefinitions(WithParameterDefinitionList(tt.ParameterDefs))
