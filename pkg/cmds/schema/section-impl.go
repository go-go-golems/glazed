...
func WithDefaults(s interface{}) SectionOption {
	return func(p *SectionImpl) error {
		...
		if m, ok := s.(map[string]interface{}); ok {
			return p.InitializeDefaultsFromFields(m)
		} else {
...
// GetDefinitions returns a map that maps all fields (flags and arguments) to their name.
...
// InitializeDefaultsFromStruct initializes the `Definition` of the section,
// which are often defined at compile time and loaded from a YAML file, with fresh
// ones from the struct.
// This is in some ways the opposite of `InitializeStructFromFieldDefaults`.
// The struct fields of `defaults` with a struct tag of `glazed` are used
// to initialize the `Definition` with a matching name. If no matching
// `Definition` is found, an error is returned.
...
// InitializeDefaultsFromFields initializes the field definitions
// of the section from the given map of field values. The field definitions
...
// It also creates a flag group representing the section and adds it to the command.
// If the section has a prefix, the flags are added with that prefix.
...
// ParseSectionFromCobraCommand parses the flags of the section from the given Cobra command.
// If the section has a prefix, the flags are parsed with that prefix (meaning, the prefix
