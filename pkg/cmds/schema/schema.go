...
// Section is a struct that is used by one specific functionality section
// to group and describe all the field definitions that it uses.
...
func WithSections(sections ...Section) SchemaOption {
	return func(pl *Schema) {
		for _, section := range sections {
			pl.Set(section.GetSlug(), section)
...
func InitializeSectionWithDefaults(
	...
	err := pds.ForEachE(func(pd *fields.Definition) error {
		...
		if v != nil {
			err := sectionValues.Fields.SetAsDefault(pd.Name, pd, v, options...)
			if err != nil {
				return err
			}
		}
...
func (pl *Schema) UpdateWithDefaults(parsedValues *values.Values, options ...fields.ParseOption) error {
	err := pl.ForEachE(func(_ string, v Section) error {
		sectionValues := parsedValues.GetOrCreate(v)
		return InitializeSectionWithDefaults(v, sectionValues, options...)
	})
