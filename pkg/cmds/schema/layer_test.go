...
func createParameterLayer(t *testing.T, slug, name string, paramDefs ...*fields.Definition) Section {
	layer, err := NewSection(slug, name, WithFields(paramDefs...))
...
func TestParameterLayersSubset(t *testing.T) {
	...
	layers := NewSchema(WithSections(layer1, layer2, layer3))
...
func TestParameterLayersForEach(t *testing.T) {
	...
	layers := NewSchema(WithSections(layer1, layer2))

	count := 0
	layers.ForEach(func(key string, p Section) {
		count++
...
func TestParameterLayersForEachE(t *testing.T) {
	...
	count := 0
	err := layers.ForEachE(func(key string, p Section) error {
		count++
		return nil
	})
...
func TestParameterLayersMerge(t *testing.T) {
	layer1 := createParameterLayer(t, "layer1", "Layer 1")
	layer2 := createParameterLayer(t, "layer2", "Layer 2")
	layers1 := NewSchema(WithSections(layer1))
	layers2 := NewSchema(WithSections(layer2))
...
func TestParameterLayersClone(t *testing.T) {
	layer1 := createParameterLayer(t, "layer1", "Layer 1")
	layers := NewSchema(WithSections(layer1))
...
func TestParameterLayersGetAllDefinitions(t *testing.T) {
	layer1 := createParameterLayer(t, "layer1", "Layer 1",
	...
	allDefs := layers.GetAllDefinitions()
...
func TestParameterLayersMergeWithOverlappingLayers(t *testing.T) {
	...
	layer2 := createParameterLayer(t, "layer2", "Layer 2")
	layers1 := NewSchema(WithSections(layer1, layer2))

	layer1Duplicate := createParameterLayer(t, "layer1", "Layer 1 - Duplicate")
	layer3 := createParameterLayer(t, "layer3", "Layer 3")
	layers2 := NewSchema(WithSections(layer1Duplicate, layer3))
