...
func createSimpleParameterLayer(t *testing.T, options ...SectionOption) *SectionImpl {
	options_ := append([]SectionOption{
		WithFields(
			fields.New("flag1", fields.TypeString),
		),
	}, options...)
	layer, err := NewSection("simple", "Simple", options_...)
...
func TestAddFlagsToCobraCommandSimple(t *testing.T) {
	...
	err := layer.AddSectionToCobraCommand(cmd)
	require.NoError(t, err)
